...
Depending on your website's architecture&mdash;particularly if your website relies heavily on client-side rendering&mdash;reducing the size of JavaScript payloads responsible for rendering markup may lead to improved [Largest Contentful Paint (LCP)](/lcp/) times in some situations. This can occur when there are resources that are only discoverable when client-side markup is rendered, or when the main thread is too busy to render that markup. These scenarios can affect an [LCP candidate's render delay](/optimize-lcp/#2-eliminate-element-render-delay).
