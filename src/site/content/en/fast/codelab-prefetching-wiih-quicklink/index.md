...
Next, add the following code to configure the `webpack-route-manifest` plugin:
...
At this point you would need to install the `quicklnk` library in your project. For simplicity, we already added it to the project. Here's the command that you would need to run:
...
First, import the quicklnk higher order component (HOC):
...
{% Aside %}
If the Glitch console throws an error at this point about the lack of a dependency, try running `refresh` to restart the server and run the `npm run build` after that.
{% endAside %}

## Measure again

1. Open the site in a new Chrome tab.
{% Instruction 'devtools-network', 'ol' %}
{% Instruction 'disable-cache', 'ol' %}
