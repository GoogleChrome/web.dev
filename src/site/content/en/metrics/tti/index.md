...
The Time to Interactive (TTI) metric measures the time from when the page starts
loading to when its main sub-resources have loaded and it is capable of reliably
responding to user input quickly.

To calculate TTI given a [performance
...
2. Search forward in time for a quiet window of at least five seconds (where
   "quiet window" is defined as: no [Long
...
[![A page load timeline showing how to compute
TTI](tti-timeline.svg)](tti-timeline.svg)

Historically, developers have optimized pages for fast render times&mdash;sometimes at
the expense of TTI.

Techniques like Server-side Rendering (SSR) can lead to scenarios where a page
_looks_ interactive (links and buttons are visible on the screen), but it's not
_actually_ interactive because the main thread is blocked, or the JavaScript
code controlling those elements hasn't loaded.
...
To avoid this problem, you should make every effort to minimize the delta
between FCP and TTI. And in cases were a noticeable delta does exist, you should
try to make it clear, visually, that the components on your page are not yet
interactive.
...
{% Aside %}
  While it is possible to measure TTI in the field, it's not recommended as user
