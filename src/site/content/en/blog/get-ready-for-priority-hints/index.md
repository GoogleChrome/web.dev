...
subhead: Priority Hints allows developers to reprioritize resource requests for better performance. Learn more about its implementation, use cases, and how you can participate in its second origin trial.
authors:
  - jlwagner
  - yoavweiss
description: Priority Hints allows developers to reprioritize resource requests for better performance. Learn more about its implementation, use cases, and how you can participate in its second origin trial.
date: 2019-02-15
update: 2020-10-20
...
Priority Hints can be set for resources in HTML by specifying an `importance` attribute on `<script>`, `<img>`, or `<link>` elements. An example use would look something like this:
...
```javascript
fetch('https://example.com/', { importance: 'low' })
  .then(data => {
...
In what circumstances might Priority Hints be useful? Let's find out.
...
<figure>
  {% Img src="image/jL3OLOhcWUQDnR4XjewLBx4e3PC3/n19NfVD9PNjq0m0jXdg7.png", alt="WebPageTest's filmstrip comparison view. At top, the Google Flights homepage renders the hero image with an initially low priority, resulting in an LCP of 2.7 seconds. At bottom, the hero image's initial priority is boosted, resulting in faster painting of the image and an LCP of 2 seconds.", width="800", height="167" %}
...
By now, you're ready to run out and use Priority Hints. But hold on first! You should be aware of a few things before you do.
...
Priority Hints is in development. Be aware that Priority Hints and their impact may change over time. Use them with this in mind. Since Priority Hints are a fine-tuning optimization, they should not cause breaking changes, but rather may cause subtly undesirable effects that may be hard to measure for some users if misused.

### Try Priority Hints

### Enabling support during the origin trial phase

Starting in Chrome&nbsp;96, Priority Hints is available as an origin trial in Chrome. The
origin trial is expected to end in Chrome&nbsp;99 (Mar 22, 2022).
