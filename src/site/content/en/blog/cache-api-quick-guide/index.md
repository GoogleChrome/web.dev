...
or a URL (`string`), and the network response is stored in the cache. If the
...
requests not in CORS mode have a status of `0`, and therefore such requests
can only be stored with `put`
...
This uses the same matching rules described in the section on retrieving.

### `cache.put`

Finally, there is `cache.put()`, which allows you to store either a response
from the network, or create and store your own `Response`. It takes to
parameters. The first is the `request`, and can either be a `Request` object
or a URL (`string`). The second is the `response`, and must be a `Response`,
either from the network, or generated by your code.
...
will allow you to store non-CORS responses, or where the status code of the
response is not in the 200 range. And will overwrite any previous responses
