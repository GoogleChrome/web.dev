...
choosing an app, then choosing who to share with. For example, I may want to
share an interesting article, either by emailing it to friends or Tweeting it to
the world.

In the past, only native apps could register with the operating sytem to receive
shares from other installed apps. The Web Share Target API allows installed web
apps to register with the underlying operating system as a share target to
...
that your app will accept. I'll describe three scenarios for the `share_target`
entry.
...
I must also add a `files` array defining the types of files my app accepts. The
...
* A photo sharing app could create a new slideshow using `title` as the
  slideshow title, `text` as a description, and `files` as the slideshow imges.
...
default), the browser opens a new window at the `action` URL. It generates a
query string using the URL encoded values supplied in the manifest. For example,
if the sharing app provides `title` and `text`, the query string is
...
Be sure to use a service worker to pre-cache the `action` page so that it will
load quickly, and will work reliably, even if the user is offline.
