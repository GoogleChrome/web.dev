...
* **Manual testing**. This is the typical method of testing, done by actual people. That means a tester will click through the application and check if it works and, at the same time, try to break it. Exploratory testing is the most common way: as the name implies, a tester explores the application, using their own knowledge to investigate the application under test with a predetermined path or any checklists.
