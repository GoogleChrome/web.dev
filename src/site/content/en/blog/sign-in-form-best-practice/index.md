...
hero: hero.jpg
alt: Closeup photo of a man holding entering data in a sign-in form on a phone
...
If users ever need to sign in to your site, then good sign-in form design is critical.
...
1. The input `name` attribute helps browsers store data for email and other input types for use with `autocomplete`. Some browsers also take note of the `id` and `type` attributes.
...
Don't add autocomplete to a new-password input in a form for creating a new account. The browser may already have a password stored for the site, and autofilling a password doesn't make sense on sign-up: for example, if two people share the same device and one wants to create a new account.
...
* Don't use the same form for sign-up and sign-in. Chrome uses the name of input fields and the form actions to fingerprint a form. If you have exactly the same form (e.g. `<form action=""><input name="username"><input name="password"><button></form>`) for sign-in and sign-up, Chrome cannot distinguish them and gets either the sign-in or the sign-up form wrong. Setting an action to /sign-in or /sign-up (even though this is an AJAX-based form) is sufficient to help.
* Avoid the "poor man’s placeholder attribute". Some websites use `<input type="text" value="Your username or email">` and `<input type="password" value="Your password"> with an focus handler that clears the value and changes the type from "text" to "password" on the second field. These things make it hard for password managers: should we override the value "Your username or email"? We would not recognize the password field.
Password change forms: Some websites don’t include a hidden username field (annotated with autocomplete="username") in their password change forms. As a result, the password manager cannot offer to save the right username on form submission.
Password change forms: For offering to save a password on a password change form, the browser needs to observe a form submission. This can be achieved by posting the form or by taking it out of the DOM. Just sending the values to the server and clearing the form is insufficient. Ideally after submitting the form successfully, the site does not show an empty form to the user because that is indistinguishable from a "failure to update password, please try again" for the browser.
