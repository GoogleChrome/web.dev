...
provide **multiple PWAs**, taking advantage of the same **domain name**, to make
the user aware that they belong to the same organization or service.

As you might have noticed, we are using different, but interrelated terms, like
domains and origins. Before moving forward, let's refresh these concepts.
...
The recommended approach in cases like these is to have each conceptually
...
each of them will manage different browser features independently, including:
...
- **Service Workers:** Each app has its own service worker(s) for the registered
  scopes.
- **Permissions:** Permissions are also scoped by origins. Thanks to that, users
  will know exactly for which service they are giving permissions for, and
...
- **Storage:** Cookies, local storage, and all forms of device-local storage are
  ...
  for one app, it will wipe all the data from the origin; there's no way of
  indicating to do this for a "single app". Note that Chrome and some other
...
separate origins" section, from the two "same-origin" options we presented,
...
"inner app"), is that all URLs in the "inner app" will actually be considered to
be part of both the "outer app" and the "inner app".
...
- **Installation Promotion:** If the user visits the "inner app" (for example,
  ...
  that it is. The workarounds for this is to install the "inner app" manually
  (via "Create Shortcut" browser menu option), or to install the "inner app"
  first before the "outer app".
- **[Notification](https://developer.mozilla.org/en-US/docs/Web/API/notification)
  and [Badging API](https://web.dev/badging-api/)**: If the "outer app" is
...
- **[Link
  ...
  "outer app"'s scope. Additionally, on Chrome OS / Android, if these apps are
...
many challenges, mainly due to the fact that the browser won't fully consider
