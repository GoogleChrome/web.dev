...
{% Aside %}
`<style scoped>` can be enabled in Chrome using the "Enable experimental WebKit features" flag in about:flags.
{% endAside %}
...
I was curious if using `:scope` in qS/qSA gives a performance boost. So… like a good engineer I threw together a [test](https://output.jsbin.com/icahoc/1/). My rationale: less surface area for the browser to do selector matching means speedier lookups.
