...
ways too implement re-authentication from [Verify phone numbers on the web with the Web OTP API](/web-otp/#is-it-safe-to-use-sms-as-a-way-to-authenticate) and the codelab [Your First WebAuthn App](https://codelabs.developers.google.com/codelabs/webauthn-reauth/#0).
{% endAside %}
...
{% Aside 'warning' %}
Some sites have password validation rules that don't allow the strong passwords generated by browser 
and third-party password managers. Make sure you're site doesn't do this, since it encourages users 
to make up their own passwords, which may be less secure.
{% endAside %}
...
## Don't disallow password pasting {: #allow-password-pasting}
...
You should, of course, also make it simple for users to reset their password if they forget it. The 
Open Web Application Security Project provides detailed guidance on [how to handle lost 
...
This enables password managers to navigate your users directly to can change their password on your 
site. This feature is now implemented in Safari, Chrome, and is coming to other browsers. [Help 
users change passwords easily by adding a well-known URL for changing 
...
For example, as well as requiring the user to set a password, you might also use require verification 
