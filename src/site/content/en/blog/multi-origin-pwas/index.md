...
description: Multi-origin architectures presents many challenges when building PWAs. In this post, we explore the good and bad uses of multiple origins, and some workarounds to build PWAs in multi-origin sites.
tags:
  - post
  - PWA
  - caching
...
- **Site sections:** Separating different sections of a site on subdomains. In news sites, it's not uncommon to see the home page at: `https://www.site.com`, while the sports section lives at `https://sports.site.com`, politics at `https://politics.site.com`, and so forth. In the case of an e-commerce site, using `https://www.site.com`, for Home, `https://listings.site.com` for Listings, `https://product.site.com` for Product Pages, for example.

- **User Flow:** Another approach that's discouraged is to separate different smaller parts of the site, like pages for the login or purchase flows in subdomains. For example, using `https://login.site.com`, and `https://checkout.site.com`.
...
The origin of the service worker script URL has to be the same as the origin of the page calling [register()](https://w3c.github.io/ServiceWorker/#navigator-service-worker-register). This means that, for example, a page at `https://www.site.com` can't call `register()` with a service worker url at `https://section.site.com`.

Another consideration is that a service worker can only control pages hosted under the origin and path it belongs to. This means that, if the service worker is hosted at `https://www.site.com` it can only control URLs from that origin (according to the path defined in the [scope parameter](https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorkerContainer/register#Parameters)), but won't control any page in other subdomains such as, for example, those in `https://section.site.com`.
...
The Cache object, indexedDB, and localStorage are also constrained to a single origin. This means it's not possible to access the caches that belong to `https://www.site.com`, from, for example: `https://www.section.site.com`.

Here are some things you can do to manage caches properly in scenarios like this:

- **Leverage browser caching:** Using [traditional browser caching best practices](https://webkit.org/blog/8090/workers-at-your-service/) is always recommended. In this scenario provides the added benefit of reusing cached resources across origins, which can't be done with the service workers cache. For best practices on how to use HTTP Cache with service workers, you can take a look at [this post](https://jakearchibald.com/2016/caching-best-practices/#the-service-worker-the-http-cache-play-well-together-dont-make-them-fight).
...
A simple workaround is to use a`start_url` with a redirect to the main origin. For example: subdomain `https://section.site.com`, can define a `start_url` of `https://section.site.com/pwa`, containing a redirect to: `https://www.site.com`, making the user always land at the home page.
...
1. The new url (i.e: `https://login.site.com`) could open inside a full screen iframe.
