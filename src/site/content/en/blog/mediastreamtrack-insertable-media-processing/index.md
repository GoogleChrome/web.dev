...
- The `MediaStreamTrackGenerator`, which consumes a stream of media frames and exposes a `MediaStreamTrack`
  interface, so that it can be used in any sink a `MediaStreamTrack` interface.
  The `MediaStreamTrackGenerator` can be provided to any sink, just like a track from `getUserMedia()`.
  It takes
  media frames as input. In addition, it provides access to control signals that are generated by the sink.
