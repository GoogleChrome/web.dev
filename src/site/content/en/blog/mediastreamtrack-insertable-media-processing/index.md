...
A remote `MediaStream` may be representing to the following outputs:

- A media element, like `<video>` or `<audio>`
- A stream originating over the network, and obtained via the WebRTC
  [`RTCPeerConnection`](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection) API
