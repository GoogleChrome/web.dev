...
subhead: A new icon format to use adaptive icons on Android.
description: |
...
If you want to create a maskable icon based off your existing transparent icon, you can use the [Maskable.app Editor](https://maskable.app/editor). Upload your icon, adjust the color and size, then export the image.
...
With the inclusion of maskable icons, a new property has been added for image resources listed in a Web App Manifest. The `purpose` field tells the browser how your icon should be used. By default, icons will have a purpose of `"any"`. These icons resized on top of a white background by the browser on Android.

Maskable icons should use a different purpose: `"maskable"`. This indicates that an image is meant to be used with icon masks, giving you more control over the result. This way, your icons will not have a white background. You can also specify multiple purposes, if you want your maskable icon to be used without a mask on other devices.
...
```json
{
  ...
  "icons": [
    ...
    {
      ...
      "sizes": "196x196",
      "type": "image/png",
      "purpose": "any maskable" // <-- New property
    }
  ]
  ...
}
...
With this, you can go forth and create your own maskable icons, making sure you app looks great edge-to-edge.
