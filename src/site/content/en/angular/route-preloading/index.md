...
You can add quicklink to an Angular app by using the [ngx-quicklink](https://www.npmjs.com/package/ngx-quicklink) package. To use `ngx-quicklink` you first have to install it from npm:
...
When you open the side navigation you'll notice that the router then preloads the ”About” route:
...
1. Pick the right preloading strategy depending on the size of your application:
    1. Applications with few modules can use Angular's built-in `PreloadAllModules` strategy.
