...
native constraints, such as `email`, `number`, `url` and date types, if the value is non-null (not empty) and the current
...
updating is different from native HTML constraint validation, which only occurs when the user attempts to submit the form.

Native constraint validation is only relevant to constraints set with HTML attributes. While you can style an element based
...
If any of the `validityState` properties return `true`, submission is blocked, and the browser displays a native error message
...
the first invalid form control will show a native error message and receive focus. If a required control has no value set, if a
...
While this section covers how HTML alone can natively provide validation client-side, validation must be both client-side and
...
and values, where necessary, and ARIA live announcements as required. But, as we’ve learned here, with HTML alone, you can get a long
way to your goal of accessibility and validity without resorting to ARIA or JS.
...
long way to your goal of accessibility and validity without resorting to ARIA or JS.
