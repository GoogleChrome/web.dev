...
The live app opens in a new Chrome tab.
...
1.  Try clicking buttons in the UI (see the Chrome dev console for output about what's going on).

    * **Register service worker** registers a service worker for the scope of your Glitch project URL.

    * **Subscribe to push** creates a push notifications subscription. It is only available when a service worker has been registered and a `VAPID_PUBLIC_KEY` constant is present in the client code (more about this later), so you can't click it just yet.
...
## Load VAPID details from environment variables

Your first TODO item is to load VAPID details. 
...
The request body contains the endpoint to remove: 
...
* Retrieve the cancelled subscription from the body of the request.
* Access the database of active subscriptions.
...
{% Aside 'success' %}
  You have implemented a push notifications server. Try it out by clicking **Notify current subscription** and **Notify all subscriptions** in the app UI.
{% endAside %}
