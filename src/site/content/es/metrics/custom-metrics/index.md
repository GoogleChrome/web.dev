...
Siempre que necesites ejecutar un código costoso (o cargar y ejecutar scripts de gran tamaño), es útil realizar un seguimiento de si ese código bloqueó o no el hilo principal. De hecho, muchas métricas de nivel superior se construyen sobre la API de Long Tasks (como [Time to Interactive (TTI): Tiempo para interactividad](/interactive/) y [Total Blocking Time (TBT): Tiempo total de bloqueo](/lighthouse-total-blocking-time/)).
