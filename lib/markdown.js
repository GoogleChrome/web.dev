 ...
 *
 * @param {string} req path name
 * @param {!Object<string, *>} config
 * @param {string} source
 * @return {string}
 */
module.exports = function(req, config, source) {
  const basename = path.basename(req);
  ...
  const projectPath = closest(req, 'en/_project.yaml').substr(2);
  const bookPath = closest(req, 'en/_book.yaml').substr(2);
