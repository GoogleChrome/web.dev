...
async function requireInternalDeployScript() {
  ...
  // as it contains Google-internal code for deploying web.dev, and is not
  // included in the public repository. It is then require-d below, so that it
  // can still include dependencies from this repo.
  const devsiteJsTarget = path.join(__dirname, 'devsite.js');
  try {
    // Explicitly unlink the file before copying in-place: the Google-internal
    // file might not have the +w permission, even though your user owns it.
    await fsp.unlink(devsiteJsTarget);
  } catch (e) {
    // ignore
  }
  try {
    ...
    throw new Error(`couldn't copy '.devsite.js' to local ver, failing deploy. Do you need to run prodaccess?`);
  }
