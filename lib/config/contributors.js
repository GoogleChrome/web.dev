...
const CONTRIBUTORS_CONFIG_LOCATION = path.resolve(__dirname, '..', '..', 'contributors', '_contributors.yaml');
const CONTRIBUTORS_PICTURES_LOCATION = path.resolve(__dirname, '..', '..', 'content', 'images', 'contributors');
...
async function loadContributorFromConfiguration(author) {
  ...
  if (!authorsConfig) {
    const contributorsFile = await fs.readFile(CONTRIBUTORS_CONFIG_LOCATION, 'utf8');
  ...
  const hasPictureFile = await fs.exists(path.resolve(CONTRIBUTORS_PICTURES_LOCATION, author + '.jpg'));
