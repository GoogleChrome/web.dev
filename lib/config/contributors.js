...
async function loadContributorFromConfiguration(author) {
  ...
  if (authorInformation.picture !== 'no-photo') {
    if (!hasPictureFile) {
      throw new Error(
        `Could not find author picture for "${author}".` +
          `(Is the picture file a "*.jpg" file and ` +
          `placed in "${CONTRIBUTORS_PICTURES_LOCATION}"?)` +
          `Set "picture: no-photo" in the "contributors.yaml" if ` +
          `you do not want your picture shown on the website.`,
      );
    }

    authorInformation.picture = author;
  }
