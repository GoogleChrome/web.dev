...
async function DevsiteIncludes(cf, cb) {
  // Find DevSite includes and ensure they are also built. Includes in DevSite
  // are relative to the top-level language directory:
  ...
  const generatedSource = source.replace(djanoIncludeRe, (part) => {
    ...
    // rewrite it to be absolute (as DevSite requires).
