...
async function AllGuidesJSON(cf) {
  ...
  // Grab all known site paths, and their configs, from under `path/blah/guides.yaml`. This just
  // fetches the files so that names can be resolved.
  const allSitePathYaml = await loader.contents(cf.rel('path/*/guides.yaml'));
