...
async function HTMLBuilder(cf) {
  if (await cf.read() !== null) {
    console.debug('failed, file exists');
  ...
  switch (config.page_type) {
    case 'guide': {
      const codelabPages = await loader.contents(cf.rel('codelabs/*.md'));
      const codelabs = [];
      for (const codelabPage of codelabPages) {
        const name = codelabPage.name;
        const config = await codelabPage.config;
        codelabs.push({
          name,
          title: config.title,
        });
      ...
      return templates('guide.html', context);
    }
  }

  cf.warn('unknown page_type, ignoring: ' + config.page_type);
  return undefined;
}
