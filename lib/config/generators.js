 ...
 * @return {{next: ?Object, topic: ?string}} next guide info, if any
 */
function findGuideInfo(sitePathConfig, fromGuide) {
  ...
  for (const {guides, title} of sitePathConfig.topics) {
    const index = guides.findIndex(({id}) => id === fromGuide);
    if (index !== -1) {
      out.next = guides[index + 1] || null;
      out.topic = title;
      break;
    }
  }
  ...
  switch (config.page_type) {
    case 'guide': {
      ...
      if (guideInfo.topic !== null) {
        context.topic = {
          title: guideInfo.topic,
          id: slug(guideInfo.topic),
        };
      }
