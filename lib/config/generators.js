...
async function AllGuidesJSONInHTML(cf) {
  ...
  return `<code id="guide-audit-mapping" style="display: none;">${JSON.stringify({guides: allGuides})}</code>`;
}
...
async function HTMLBuilder(cf) {
  ...
  // Define default DevSite 2 meta content. Note that this is strictly key/value pairs.
  const meta = {
    'project_path': `/${closest(cf.path, '_project.yaml')}`,
    'book_path': `/${closest(cf.path, '_book.yaml')}`,
  };
