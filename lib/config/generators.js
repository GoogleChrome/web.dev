...
async function configForSitePath(loader, sitePath, originalCf=null) {
  ...
  for (const topic of config.topics || []) {
    ...
    if (guides.length) {
      out.topics.push({title, guides, num: ++count});
    }
  }
 ...
 * @return {?Object} next guide info, if any
 */
function findNextGuide(sitePathConfig, fromGuide) {
  for (const {guides} of sitePathConfig.topics) {
    const index = guides.findIndex(({id}) => id === fromGuide);
    if (index !== -1) {
      return guides[index + 1] || null;
    }
  }
...
async function ContentPageBuilder(cf) {
  ...
  switch (config.page_type) {
    case 'guide': {
      ...
      const next = findNextGuide(sitePathConfig, guideId);
