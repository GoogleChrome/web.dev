 ...
 * This is strictly key/value pairs.
 *
...
/**
 * Loads basic config, title and href to all artifacts found in a guide
 * directory.
...
/**
 * Generates data about all guides, for the profile page.
 * Internally, this matches the object typed as `GuideToAuditMapping`.
 *
...
async function AllGuidesJSON(cf) {
  ...
  // Grab all known site paths, and their configs,
  // from under `blah/guides.yaml`.
  // The existence of `guides.yaml` in a subfolder predisposes a path.
  // This just fetches the files so that names can be resolved.
  ...
  // For all paths we found,
  // iterate through all their guides (defined in `blah/guides.yaml`)
...
async function MarkdownBuilder(cf, cb) {
  ...
  // If `page_type` wasn't specified,
  // just generate regular Markdown stripped of its config.
  let header = '';
  if (config.title) {
    header += `# ${config.title}\n\n`;
  }
...
/**
 * Generates the named `pathName.html` page
 * based on its corresponding config YAML.
 *
 * @param {!ContentFile} cf
 * @return {?string}
 */
async function PathPageBuilder(cf) {
  ...
  if (peer === null) {
    // This isn't a path page: we expect to build e.g. "foo.html",
    // and it to have a corresponding "foo/guides.yaml" file. Skip.
    return undefined;
  }
  ...
  for (const {title, guides} of config.topics || []) {
    // Generate builtGuides which contains read information from the guides
    // that are referenced in our YAML.
    // This is done with a for loop as we need to await for loads.
    const builtGuides = [];
    for (const id of guides) {
      ...
      const codelabs = (await artifactsForGuide(
        loader,
        guideIndexPage.dir,
  ...
  // Render inner `path.html` template,
  // then render its content inside `devsite.html`.
...
/**
 * Generates 'content pages', content that is located within a site path.
 * This represents the bulk of author-generated content.
 *
 * @param {!content.ContentFile} cf
 * @return {string|undefined}
 */
async function ContentPageBuilder(cf) {
  ...
  // This is the left-most part of the total path
  // (e.g. `accessible/blah/foo.html` => `accessible`,
  // assuming that `accessible/guides.yaml` exists).
  // TODO(samthor): This is relatively slow (0.5s => 1.5s build)
  // as we have LOTS of these content pages.
  // Perhaps memoize per-path (as we only have ~6 in total)?
  ...
  const content = await peer.content;
  const config = await peer.config;
  ...
  switch (config.page_type) {
    case 'guide': {
      ...
      // e.g. ContentFile "fast/foo/index.md" => "foo"
      const guideId = path.basename(cf.dir);
      const guideInfo = findGuideInfo(sitePathConfig, guideId);

      // Render a Guide. This contains its markdown,
      // and links to related peer artifacts found in its current path.
      // Fetch all neighbouring Markdown files,
      // and create links to them if they match types we support
      // (currently just "glitch", for Codelabs).
      const codelabs = (await artifactsForGuide(loader, cf.dir)).filter(
        ({type}) => type === 'glitch',
      );
      const context = {
        flags,
      ...
      // It's possible although unlikely that this guide is not
      // actually part of a path's topics.
      ...
      if (typeof config.author === 'string') {
        context.author = await loadContributorFromConfiguration(config.author);
      }
    ...
    case 'glitch': {
      ...
      // Fetch the related Guide for this codelab so it can be linked back to.
