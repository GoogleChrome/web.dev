...
async function AuditGuidePaths(loader, cf) {
  ...
  const pathPages = await loader.contents(path.join(cf.dir, 'path/*.md'));
...
async function MarkdownBuilder(loader, cf) {
  ...
  const projectPath = closest(cf.path, '_project.yaml');
  const bookPath = closest(cf.path, '_book.yaml');
