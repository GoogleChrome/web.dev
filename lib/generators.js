...
async function MarkdownBuilder({cf, push}, out) {
  // generate final Markdown based on YAML prefix config
  out = markdown(cf.path, await cf.config, out);
...
async function FilterMarkdown({cf}) {
