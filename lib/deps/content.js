...
class ContentFile {
  ...
  _prepareRead() {
    ...
    this._config = this._config ? this._config.then(YAML) :  Promise.resolve(null);
  }
...
class ContentLoader {
  ...
  async _dirContents(rootDir, request, recurse) {
    const pendingDir = [rootDir];
    const all = [];

    while (pendingDir.length) {
      const currentDir = pendingDir.shift();
      const out = await this._singleDirContents(currentDir, request)
  ...
  async _singleDirContents(currentDir, request) {
    const subdir = [];
    const all = new Map();
