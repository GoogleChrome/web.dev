...
/**
 * @typedef {function(!ContentFile): (string|!Array<string>|undefined|null)}
 */
...
class ContentFile {
   ...
   *
   * @param {function(!ContentFile): void} other
...
class VirtualContentFile extends ContentFile {
  ...
  async build(other) {
    ...
    try {
      ...
      const gen = this._gen(this._parent);
      this._build = gen.then((out) => {
        // ... look for special-case and pass to others
        if (out instanceof Array) {
          // Generate ContentFile instances for all other wanted files, pass to callback.
          const files = out.map((cand) => {
            const joined = this.rel(cand);
            const all = this.loader.build(joined);
            all.forEach((cf) => other(cf))
          });
          out = undefined;  // defer to parent
        }
    ...
    if (virtual) {
      ...
      for (const {glob, gen} of generators) {
        ...
        if (generatedMatch !== null) {
          ...
          matches = [...all.keys()].map((raw) => requestMatchGlob(raw, baseGlob));
        }
