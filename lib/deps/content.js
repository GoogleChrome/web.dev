...
class VirtualContentFile extends ContentFile {
  ...
  async build() {
    ...
    const otherHandler = (cand) => {
      generatedCandPromises.push(this.loader._dependency(this, cand));
    };
    ...
    try {
      ...
      this._build = gen.then(async (out) => {
        ...
        // ... if returns undefined, this means defer to the parent build step
        if (out === undefined) {
...
class ContentLoader {
  ...
  /**
   * @param {!ContentFile} cf the ContentFile which implied the dependency
   * @param {string} cand relative candidate path(s) to include
   */
  async _dependency(cf, cand) {
    const all = await this.build(cf.rel(cand));
