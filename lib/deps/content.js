...
class ContentLoader {
  ...
  _virtual(candidateDir, candidateName='*') {
    ...
    const out = this._gen.map(({cand, gen}) => {
      ...
      if (micromatch.isMatch(candidateName, base)) {
      ...
      if (!isGlob(base)) {
        if (candidateName === '*' || micromatch.isMatch(base, candidateName)) {
          return {base, gen};
        }
      }
  ...
  async _dirContents(rootDir, globName, recurse) {
    ...
    while (pendingDir.length) {
      ...
      for (const {base: raw, gen} of virtualFiles) {
        seen.add(raw);
