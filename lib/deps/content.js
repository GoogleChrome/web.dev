 ...
 * @param {string} glob that was registered on this loader (may be fixed)
...
/**
 * ContentFile is a buildable node in the build system,
 * mapping directly to one file, which may be a completely generated file.
 *
 ...
 *   + VirtualContentFile represents a generation step that internally points
 *     to any other type of step (including another instance of itself).
 */
class ContentFile {
  /**
  ...
  /**
   * @return {boolean} whether this file was imagined and
   * does not actually appear on-disk
   */
  get empty() {
    return false;
  }
  ...
  /**
   * @return {string} the actual path DevSite will serve this file under,
  ...
  /**
   * Returns what should be written to disk:
   * a string, undefined (the real file), or null (delete).
   *
...
class ContentLoader {
  /**
   * @param {function(string): void} cb called when a file requires
   * another to be built
   */
  constructor(cb) {
    this._cb = cb;
  ...
  /**
   * Registers a glob,
   * with paths matching the path being passed to the specified generator.
   *
   * If the specified name is a glob,
   * then this will only create virtual files when explicitly specified or
   * included as a dependency.
   *   e.g. 'example/_config-*.yaml' will not match for 'example/*',
   *        but will for an explicit path such as 'path/to/_config-foo.yaml'.
   *
  ...
  register(glob, gen) {
    ...
    if (typeof gen !== 'function') {
      throw new TypeError(
          `cannot register non-function generator for: ${glob}, ${gen}`
      );
    }
  ...
  _genForDir(candidateDir) {
    ...
    return this._gen.filter(
        ({glob}) =>
          glob.startsWith('./') ||
        micromatch.isMatch(candidateDir, path.dirname(glob))
    );
  }
  ...
  async contents(req, opts = {}) {
    ...
    // this is a directory glob, like "foo/*/bar", split into parts:
    // walk from left and expand glob parts by recursively calling .contents()
    ...
    while (parts.length) {
      ...
      // TODO(samthor): If we are gen-ing a file in a virtual dir,
      // it should match here too:
  ...
  async _dirContents(rootDir, request, opts) {
    ...
    while (pendingDir.length) {
      ...
      const out = await this._singleDirContents(
          currentDir,
          request,
          opts.virtual
      );
    ...
    if (virtual) {
      ...
      for (const {glob, gen} of generators) {
        ...
        if (generatedMatch !== null) {
          ...
          // ... or match many files
          // (where the glob is actually a glob, e.g. "*.md")
          matches = [...all.keys()]
              .map((raw) => requestMatchGlob(raw, baseGlob))
              .filter(Boolean);
        }
        for (const match of matches) {
          // link a new VirtualContentFile to any previous ContentFile
          // with this name, and return it instead
   ...
   * @return {!ContentFile} returns the ContentFile for this real path,
   * possibly from cache
   */
