...
describe('live-posts', () => {
  ...
  it('should return false if the post has a future date (scheduled posts)', () => {
  ...
  it('should return true if the post has a past date (scheduled posts)', () => {
  ...
  it('should return false if the post is a draft', () => {
    post.data.draft = true;
    const actual = livePosts(post);
    assert.strictEqual(actual, false);
  });

  it('should return true if the post is not a draft', () => {
    const actual = livePosts(post);
    assert.strictEqual(actual, true);
  });

  it('should return false if the post has a past date but is a draft', () => {
    // set the post date to 1 day in the past.
    post.date.setDate(post.date.getDate() - 1);
    post.data.scheduled = true;
    post.data.draft = true;
  ...
  describe('isScheduledForTheFuture', () => {
    it('should throw if now argument is not a Date', () => {
      assert.throws(
